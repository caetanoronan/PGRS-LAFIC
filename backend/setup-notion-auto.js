...
if (!notionApiKey) {
  ...
  console.log('Uso: node setup-notion-auto.js <your_notion_token>');
  process.exit(1);
}

if (!notionApiKey.startsWith('ntn_')) {
  console.error('❌ Token inválido. Deve começar com "ntn_"');
  process.exit(1);
}
...
async function createDatabases() {
  ...
  // Criar página pai para as tabelas
  console.log('📄 Criando página pai para as tabelas...');
  const parentPage = await notion.pages.create({
    parent: { type: 'workspace', workspace: true },
    title: [{ type: 'text', text: { content: 'PGRS LAFIC - Gerenciamento de Resíduos' } }],
    icon: { type: 'emoji', emoji: '♻️' }
  });
  console.log(`✅ Página criada: ${parentPage.id}\n`);
...
`;
    ...
    if (error.code === 'invalid_request_body') {
      console.error('💡 Dica: Certifique-se de que o token está correto e válido');
    }
    process.exit(1);
  }
}
